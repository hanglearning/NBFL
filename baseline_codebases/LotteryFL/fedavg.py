...
from model.cifar10.cnn import CNN as CNNCifar
from model.mnist.cnn import CNN as CNNMnist
from baseline_codebases.LotteryFL.lotteryfl_utils import get_dataset, get_dataset_femnist, get_dataset_cifar10_extr_noniid, get_dataset_HAR, get_dataset_HAD, get_dataset_mnist_extr_noniid, average_weights, average_weights_with_masks, exp_details, make_mask, prune_by_percentile, mask_model, mix_global_weights, get_dataset_miniimagenet_extr_noniid
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idxs_users:
            ...
            w, loss = local_model.update_weights(
                model=copy_model(global_model, device=device), global_round=epoch, device=device)
            # get new model
            new_model = copy_model(global_model, device=device)
