...
for attack_type in [0, 4]:
    for mal in [0, 3, 6, 10]:
        for alpha in [1.0, 100.0]:
            ...
            legend_handles = [
                Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label=f'Forking Event, alpha = {alpha}')
            ]
    ...
    plt.savefig(f'{log_base_path}/LBFL/logs/forking_events_alpha_{alpha}.png', dpi=300)
    plt.clf()
    # plt.show()